...
acceptance("Docs Card Filter | tag route", function (needs) {
  ...
  test("tag route provides tag name for filtering", async function (assert) {
    ...
    assert.strictEqual(
      tagName,
      "important",
      "tag.name is available for card filter logic"
    );
  });
});
